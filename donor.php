            ...
            <form id="formPostDonation">
                ...
                <div class="grid-cols-2">
                    <div class="form-group">
                        <label class="form-label">Material / Item Name</label>
                ...
                <div class="grid-cols-2">
                    <div class="form-group">
                        ...
                        <input type="number" class="form-control" id="donQuantity" min="1" value="1" required>
                    </div>
                    <div class="form-group">
                        <label class="form-label">Condition</label>
                        <select class="form-control form-select" id="donCondition" required>
                ...
                <div class="form-group">
                    <label class="form-label">Item Photo (PNG/JPG)</label>
                ...
                <div class="form-group">
                    <label class="form-label">Availability Date & Delivery Method</label>
                    <div class="grid-cols-2">
                        <input type="date" class="form-control" id="donAvailability">
                        <select class="form-control form-select" id="donDeliveryMethod">
                            <option value="donor_delivery">Donor Will Deliver to Receiver</option>
                            <option value="receiver_pickup">Receiver Pickup Required</option>
                ...
                <div class="form-group">
                    <label class="form-label">Description / Special Handover Notes</label>
                    <textarea class="form-control" id="donDescription" rows="2" placeholder="Enter expiration date, weight, dimensions, or pickup instructions..."></textarea>
                </div>

                <button class="btn btn-primary" type="submit" id="formSubmitBtn" style="width: 100%;">
                    Post Physical Donation Listing
                </button>
            </form>
        </div>
...
<div id="listings-panel" class="dashboard-view-panel" style="display: none;">
    <div class="glass-panel" style="padding: 30px; margin-bottom: 24px; background: #FFFFFF;">
        <div class="card-header">
            <h3 class="card-title">Manage Offered Material Listings</h3>
        </div>
...
<div id="needs-catalogue-panel" class="dashboard-view-panel" style="display: none;">
    <div class="glass-panel" style="padding: 30px; margin-bottom: 24px; background: #FFFFFF;">
        <div class="card-header">
            <h3 class="card-title">Browse Verified Receiver Requests</h3>
            <span style="font-size: 0.8rem; color: var(--color-primary); font-weight: 700;">APPROVED REQUESTS</span>
        </div>
        
        <!-- Multi-Filter Control Toolbar -->
        <div class="glass-panel" style="padding: 16px; margin-bottom: 24px; background: #FBF5DD;">
            <div class="grid-cols-3" style="gap: 16px; margin-bottom: 12px;">
                <div>
                    <label class="form-label" style="font-size: 0.75rem;">Keyword Search</label>
                    <input type="text" id="searchDonorNeeds" class="form-control" placeholder="Search requests...">
                </div>
                <div>
                    <label class="form-label" style="font-size: 0.75rem;">Request Type</label>
                    <select id="filterReqType" class="form-control form-select">
            ...
            <div class="grid-cols-3" style="gap: 16px;">
                <div>
                    <label class="form-label" style="font-size: 0.75rem;">District / Location</label>
                    <select id="filterReqDistrict" class="form-control form-select">
                ...
                <div style="display: flex; align-items: flex-end;">
                    <button class="btn btn-secondary" id="btnResetFilters" style="width: 100%; padding: 10px;">
                        Reset Filters
                    </button>
                </div>
            </div>
...
<div id="chat-panel" class="dashboard-view-panel" style="display: none;">
    <div class="dashboard-grid" style="grid-template-columns: 1.1fr 1.4fr; gap: 20px;">
        <div style="display: flex; flex-direction: column; gap: 20px;">
            ...
            <div class="glass-panel" style="padding: 24px; background: #FFFFFF;">
                <div class="card-header" style="margin-bottom: 16px; padding-bottom: 10px;">
                    <h3 class="card-title">Support Progress Milestones</h3>
                </div>
        ...
        <div class="glass-panel chat-widget">
            <div class="chat-header">
                ...
                <div>
                    <h4 style="font-size: 0.95rem;" id="chatPeerName">Select Recipient</h4>
...
<!-- MODAL DIALOG: MONETARY DONATION OFFER -->
<div class="custom-modal-backdrop" id="modalMonetaryDonation">
    <div class="custom-modal-card">
        <h3 style="color: var(--color-primary); margin-bottom: 12px;" id="mdlMonTitle">Monetary Transfer Details</h3>
        <p style="font-size: 0.9rem; color: var(--color-text-muted); margin-bottom: 16px;">
            Transfer directly to the receiver's official verified bank account, then upload your transaction receipt.
        </p>

        <div class="glass-panel" style="padding: 16px; background: #FBF5DD; margin-bottom: 20px; font-size: 0.85rem; border: 1px solid var(--color-border-dark);">
            <div style="font-weight: 700; color: var(--color-primary); margin-bottom: 6px;">Verified Receiver Bank Details:</div>
            <div id="mdlBankDetailsContent">Loading bank info...</div>
        </div>

        <form id="formSubmitMonetaryDonation">
            <input type="hidden" id="mdlMonRequestId" value="">
            <div class="grid-cols-2">
                <div class="form-group">
                    <label class="form-label">Transfer Amount (LKR)</label>
                    <input type="number" class="form-control" id="mdlMonAmount" required min="100">
                </div>
                <div class="form-group">
                    <label class="form-label">Transfer Date</label>
                    <input type="date" class="form-control" id="mdlMonDate" required>
                </div>
            </div>

            <div class="form-group">
                <label class="form-label">Transaction Reference Number</label>
                <input type="text" class="form-control" id="mdlMonRef" placeholder="e.g. TXN-98765432" required>
            </div>

            <div class="form-group">
                <label class="form-label">Transaction Receipt (PNG/JPG/PDF)</label>
                <input type="file" id="monReceiptUploadInput" class="form-control" accept=".pdf,.png,.jpg,.jpeg" required>
                <input type="hidden" id="mdlMonReceiptUrl" value="">
            </div>

            <div style="display: flex; gap: 12px; justify-content: flex-end; margin-top: 24px;">
                <button type="button" class="btn btn-secondary" onclick="closeCustomModal('modalMonetaryDonation')">Cancel</button>
                <button type="submit" class="btn btn-primary">Confirm Monetary Offer</button>
            </div>
        </form>
    </div>
</div>

<!-- MODAL DIALOG: VOLUNTEER OFFER -->
<div class="custom-modal-backdrop" id="modalVolunteerOffer">
    <div class="custom-modal-card">
        <h3 style="color: var(--color-primary); margin-bottom: 12px;" id="mdlVolTitle">Volunteer Support Offer</h3>
        <p style="font-size: 0.9rem; color: var(--color-text-muted); margin-bottom: 16px;">
            Specify your availability, skills, and any equipment you can bring to assist the receiver organisation.
        </p>

        <form id="formSubmitVolunteerOffer">
            <input type="hidden" id="mdlVolRequestId" value="">
            
            <div class="grid-cols-2">
                <div class="form-group">
                    <label class="form-label">Number of Volunteers Attending</label>
                    <input type="number" class="form-control" id="mdlVolCount" min="1" value="1" required>
                </div>
                <div class="form-group">
                    <label class="form-label">Your Availability / Shift</label>
                    <input type="text" class="form-control" id="mdlVolAvailability" placeholder="e.g. Full Day / Morning Shift" required>
                </div>
            </div>

            <div class="form-group">
                <label class="form-label">Relevant Skills / Experience</label>
                <input type="text" class="form-control" id="mdlVolSkills" placeholder="e.g. First Aid, Carpentry, Painting">
            </div>

            <div class="glass-panel" style="padding: 16px; background: #FBF5DD; border: 1px solid var(--color-border-dark); margin-bottom: 20px;">
                <h4 style="font-size: 0.85rem; color: var(--color-primary); margin-bottom: 6px;">Equipment Support (Optional)</h4>
                <div style="font-size: 0.8rem; color: var(--color-text-muted); margin-bottom: 10px;" id="mdlVolEquipmentRequiredNotice">
                    Specify any required equipment you can bring and available quantities.
                </div>

                <div class="form-group" style="margin-bottom: 0;">
                    <label class="form-label" style="font-size: 0.75rem;">Equipment Provided & Quantities</label>
                    <input type="text" class="form-control" id="mdlVolEquipmentProvided" placeholder="e.g. 2 Ekel Brooms, 1 Garden Cutter">
                </div>
            </div>

            <div style="display: flex; gap: 12px; justify-content: flex-end; margin-top: 24px;">
                <button type="button" class="btn btn-secondary" onclick="closeCustomModal('modalVolunteerOffer')">Cancel</button>
                <button type="submit" class="btn btn-primary">Submit Volunteer Offer</button>
            </div>
        </form>
    </div>
</div>
